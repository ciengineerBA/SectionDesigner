...
namespace Arayuz
{
    class SteelMaterial
    {
        ...
        public double StressDy2018(double strain)
        {
            ...
            if (Math.Abs(st) < Fy*fexfk / E)
            {
                return str = st * E;
            }
            else if ((Fy * fexfk / E )< Math.Abs(st) && Math.Abs(st) < Esh)
            {
                return str = Fy* fexfk*Math.Sign(st);
            }
            else
            {
                st = Math.Abs(st);
                 str = Fult* fexfk - (Fult* fexfk - Fy * fexfk) * Math.Pow(Eult - st, 2) / Math.Pow(Eult - Esh, 2);
